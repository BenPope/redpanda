...
struct protobuf_schema_definition::impl {
    ...
    void render_nested(
      ...
      int indent) const {
        ...
        for (const auto& field : fields) {
            ...
            } else if (field.type() == pb::FieldDescriptorProto::TYPE_GROUP) {
                break;
            } else {
                has_fields = true;
        ...
        auto nested_messages = boost::adaptors::filter(
          message.nested_type(),
    ...
    render_imports(std::ostream& os, const pb::FileDescriptorProto& fdp) const {
        ...
        auto is_public = [&](const auto& dep) {
            return absl::c_any_of(fdp.public_dependency(), [&](int j) {
                return fdp.dependency()[j] == dep;
            });
        };
        auto is_weak = [&](const auto& dep) {
            return absl::c_any_of(fdp.weak_dependency(), [&](int j) {
                return fdp.dependency()[j] == dep;
            });
        };
