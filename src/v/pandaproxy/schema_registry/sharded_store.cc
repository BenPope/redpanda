...
sharded_store::has_schema(const referenced_schema& ref) {
...
ss::future<bool> sharded_store::is_compatible(
  schema_version version, const referenced_schema& new_schema) {
    ...
    if (old_schema.ref.type() != new_schema.type()) {
        co_return false;
    }
    ...
    auto new_avro = make_avro_schema_definition(new_schema.raw_def()()).value();
    auto is_compat = true;
