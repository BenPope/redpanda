...
private:
    ss::future<clock::duration> do_record_partition_mutations(
      std::optional<std::string_view> client_id,
      uint32_t mutations,
      clock::time_point now);

    clock::duration cap_to_max_delay(
      std::optional<std::string_view> client_id, clock::duration delay_ms);
...
private:
    ...
    using quota_mutation_callback_t
      = std::function<clock::duration(client_quota&)>;
