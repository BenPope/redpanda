...
// atomic_token_bucket tracks the rate of a metric over time using a atomic
// counters allowing, limiting taking tokens from the bucket it goes over the
// configured rate.
//
// atomic_token_bucket -- configures whether the bucket can grow indefinitely
// (capped_release::no) or if the bucket is capped to a maximal burst rate set
// by the limit constructor argument (capped_release::yes)
template<ss::internal::capped_release capped_release>
class atomic_token_bucket final {
public:
    ...
    /// if the bucket is capped (capped_release::yes), or the maximal increment
    /// we allow replenishing the bucket with if the bucket is uncapped
    /// (capped_release::no)
...
private:
    ...
    using bucket_t = ss::internal::
      shared_token_bucket<uint64_t, std::ratio<1>, capped_release, clock>;
