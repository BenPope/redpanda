 ...
 *
 * It hashes the same as a topic_partition_view.
 *
 * It
 */
class kitp_view {
...
public:
    ...
    kitp_view as_kitp_view() const { return {_id, _ktp.as_tp_view()}; }

    /**
     * @brief Returns a the ktp's topic_id.
     *
     ...
     * in non-release builds.
     */
    partition_id get_partition() const {
        check();
        return _ktp.get_partition();
    }

    topic_namespace_view as_tn_view() const {
        return {kafka_namespace, _ktp.get_topic()};
    }

    ktp as_ktp() const { return {_ktp.get_topic(), _ktp.get_partition()}; }
