...
#include "json/json.h"
#include "meta/fmt/fmt_format.h"
#include "meta/fmt/std_format.h"
...
struct formatter<foo::address> : formatter<string_view> {
    ...
    auto format(foo::address const& a, FormatContext& ctx) const {
        return fmt::format_to(ctx.out(), "host: {}, port: {}", a.host, a.port);
    }
};
...
BOOST_AUTO_TEST_CASE(test_fmt) {
    ...
    // Fails to compile!
    // std::format("{}", foo::address{"localhost", 8080});
    ...
    BOOST_REQUIRE_EQUAL(
      std::format("{}", foo::meta_address{"localhost", 8080}), expected);
}
