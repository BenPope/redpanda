...
class AccessControlListTest(RedpandaTest):
    ...
    def prepare_cluster(self,
        ...
        self.security.enable_authorization = enable_authz
    ...
    # authorization. New config style has enable_authorization at the
    # top-level, with authentication_method on the listener.
    @cluster(num_nodes=3)
    # plaintext conn + sasl for authn (old config)
    @parametrize(use_tls=False,
                 use_sasl=True,
                 enable_authz=None,
                 authn_method=None)
    # ssl/tls conn + sasl for authn (old config)
    @parametrize(use_tls=True,
                 use_sasl=True,
                 enable_authz=None,
                 authn_method=None)
    # ssl/tls conn + mtls for authn (old config, GA mtls config)
    @parametrize(use_tls=True,
                 use_sasl=False,
                 enable_authz=None,
