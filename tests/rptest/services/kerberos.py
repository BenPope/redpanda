...
class KrbClient(Service):
    ...
    def metadata(self, principal: str):
        ...
        try:
            res = self.nodes[0].account.ssh_output(
                cmd=
                f"KRB5_TRACE=/dev/stderr KRB5CCNAME={client_cache} kcat -L -J -b {self.redpanda.kerberos_brokers()} {sasl_conf}",
                allow_fail=False,
    ...
    def metadata_java(self, principal: str):
        ...
        try:
            cmd_args = f"--command-config {properties_filepath} --bootstrap-server {self.redpanda.kerberos_brokers()}"
